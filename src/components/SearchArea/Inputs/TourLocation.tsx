...
const LocationOption: FC<TourLocationType> = ({ name, onClick }) => {
  return (
...
const TourLocation: FC<{
  label: string;
  compact?: boolean;
  value?: TourLocationType;
  onChange: (value: TourLocationType) => void;
  ...
  const [list, listHandlers] = useListState<TourLocationType>([]);
  ...
  const Target = (
    ...
    >
      ...
      {!compact && (
        <Text
          size="xl"
          fw={!value?.name ? 600 : 700}
          c={!value?.name ? "gray.5" : undefined}
          truncate
        >
          {value?.name || t(label)}
        </Text>
